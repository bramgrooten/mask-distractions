...
def make_env(
	...
	assert mode in {'clean', 'color_easy', 'color_hard', 'video_easy', 'video_hard', 'distracting_cs'}, \
		f'specified mode "{mode}" is not supported'

	paths = []
	mode = 'train' if mode == 'clean' else mode
...
class VideoWrapper(gym.Wrapper):
	...
	def _load_video(self, video):
		...
		# Check if the opened video cap has appropriate dimensions
		assert cap.isOpened(), f'Could not open video {video}. Check whether the working directory is set to the project root.'
